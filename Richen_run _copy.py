...
#Richen
#update: Apr/20
def get_f1_metrics(real_preds, sample_preds):
    #precision, recall, _ = precision_recall_curve([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    #f1_score = 2 * (precision*recall) / (precision+recall)
    f1_score = f1_score([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds, pos_label=None, average=None)
    return f1_score.tolist()

    
def get_accuracy(real_preds, sample_preds):
    return accuracy_score([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    

def get_confusion_matrix(real_preds, sample_preds):
    confusion_m = confusion_matrix([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds, normalize='true')
    return confusion_m
    #return confusion_m.tolist()

def vis_confusion_matrices(experiment):
    confusion_mat = experiment["confusion_matrix"]
    group_names = ['True Neg', 'False Pos', 'False Neg', 'True Pos']
    group_vals = ["{0:.2%}".format(value) for value in confusion_mat.flatten()]
    labels = [f"{v1}\n{v2}" for v1, v2 in zip(group_names, group_vals)]
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500):
    ...
    #Richen
    #update: Apr/20
    f1 = get_f1_metrics(predictions['real'], predictions['samples'])
    accur = get_accuracy(predictions['real'], predictions['samples'])
    confusion_mat = get_confusion_matrix(predictions['real'], predictions['samples'])
