...
# TODO Test if this partial noise work or not
class AppendedNoiseLayer(nn.Module):
    """ Custom layer that appends a Noise Layer to the original layer"""
    def __init__(self, layer, noisy_level, pct_words_masked=0.3):
        super().__init__()
        self.original = deepcopy(layer)
        self.noisy_level = noisy_level
        self.pct_words_masked = pct_words_masked

    def forward(self, x):
        noise = ((torch.rand_like(x).cpu()-0.5) * self.noisy_level) * torch.bernoulli(torch.ones(x.shape[0:2]) * self.pct_words_masked).unsqueeze(2) # TODO: RuntimeError: Expected all tensors to be on the same device, but found at least two devices, cuda:0 and cpu!
        original_out = self.original(x).to("cuda")
        noise = noise.to("cuda")
        return original_out + noise
...
def get_ll_noise_last(text, n_perturbations=1, noisy_level=0.3, pct=0.3):
    """
    Use perturbation noise_last method.
    """
    
    if not args.openai_model:
        tokenized = base_tokenizer(text, return_tensors="pt").to(DEVICE)
        labels = tokenized.input_ids
        ll = -base_model(**tokenized, labels=labels).loss.item()
        p_lls = []
        for _ in range(n_perturbations):
            transformer_outputs = base_model.transformer(**tokenized, output_attentions=True, output_hidden_states=True)

            hidden_states = transformer_outputs[0]

            noise = torch.randn_like(hidden_states) * noisy_level
            hidden_states = ((noise + hidden_states).detach() - hidden_states).detach() + hidden_states

            hidden_states = hidden_states.to(base_model.lm_head.weight.device)
            
            lm_logits = base_model.lm_head(hidden_states)

            labels = tokenized.input_ids.to(lm_logits.device)
            ...
            p_lls.append(loss.item())
    else:
        raise Exception("Noise_last perturbation for openai model not implementated yet!")
    
    return ll, p_lls

def get_ll_noise_embed(text, n_perturbations=1, noisy_level=0.3, pct=0.3):
    """
    Use perturbation noise_embed method.
    """
    if not args.openai_model:
        tokenized = base_tokenizer(text, return_tensors="pt").to(DEVICE)
        labels = tokenized.input_ids
        ...
        base_model.transformer.drop = AppendedNoiseLayer(base_model.transformer.drop, noisy_level, pct)
        for _ in range(n_perturbations):
            ll = -base_model(**tokenized, labels=labels).loss.item()
            p_lls.append(ll)
        base_model.transformer.drop = base_model.transformer.drop.original
    else:
        raise Exception("Noise_embed perturbation for openai model not implementated yet!")
    return ll, p_lls
...
def get_threshold(real_preds, sample_preds, target_fpr=0.01):
    preds = real_preds + sample_preds
    true_label = [0] * len(real_preds) + [1] * len(sample_preds)
    fpr, tpr, threshold = roc_curve(true_label, preds)
    indices = None
    ...
    tpr_value = np.mean([tpr[i] for i in indices])
    threshold_value = np.mean([threshold[i] for i in indices])
    
    pred_label = [0] * len(preds)
    for i in range(len(preds)):
        if (preds[i] >= threshold_value):
            pred_label[i] = 1
    return tpr_value, threshold_value, pred_label

#Richen
#update: Apr/20
def get_f1_metrics(real_preds, sample_preds, pred_label):
    #precision, recall, _ = precision_recall_curve([0] * len(real_preds) + [1] * len(sample_preds), real_preds + sample_preds)
    #f1_score = 2 * (precision*recall) / (precision+recall)
    #_, threshold_val, pred_label = get_threshold(real_preds, sample_preds, target_fpr)
    f1_score_ = f1_score([0] * len(real_preds) + [1] * len(sample_preds), pred_label, pos_label=None, average=None)
    return f1_score_.tolist()
    
def get_accuracy(real_preds, sample_preds, pred_label):
    #_, threshold_val, pred_label = get_threshold(real_preds, sample_preds, target_fpr)
    return accuracy_score([0] * len(real_preds) + [1] * len(sample_preds),pred_label)
    
def get_confusion_matrix(real_preds, sample_preds, pred_label):
    #_, threshold_val, pred_label = get_threshold(real_preds, sample_preds, target_fpr)
    confusion_m = confusion_matrix([0] * len(real_preds) + [1] * len(sample_preds),pred_label, normalize='true')
    #return confusion_m
    return confusion_m.tolist()
#end
...
def get_perturbation_results(span_length=10, n_perturbations=1, n_samples=500, perturb_method="detectgpt"):
    ...
    if perturb_method in ["detectgpt", "one-model"] :
        load_mask_model()
    ...
    elif "noise" in perturb_method:
        perturb_fn = get_ll_noise_last if args.perturb_method == "noise_last" else get_ll_noise_embed
        ...
        for res in tqdm.tqdm(results, desc="Computing log likelihoods"):
            res["original_ll"], res["all_perturbed_original_ll"] = perturb_fn(res["original"], n_perturbations, noisy_level, pct)
            res["sampled_ll"], res["all_perturbed_sampled_ll"] = perturb_fn(res["sampled"], n_perturbations, noisy_level, pct)
...
def run_perturbation_experiment(results, criterion, span_length=10, n_perturbations=1, n_samples=500, target_fpr=0.01):
    ...
    # print("Debugging: predictions:", predictions) #debugging
    
    fpr, tpr, roc_auc = get_roc_metrics(predictions['real'], predictions['samples'])
    p, r, pr_auc = get_precision_recall_metrics(predictions['real'], predictions['samples'])
    name = f'perturbation_{n_perturbations}_{criterion}'
    print(f"{name} ROC AUC: {roc_auc}, PR AUC: {pr_auc}")
    _, threshold_val, pred_label = get_threshold(predictions['real'], predictions['samples'], target_fpr)
    
    f1 = get_f1_metrics(predictions['real'], predictions['samples'], pred_label)
    accur = get_accuracy(predictions['real'], predictions['samples'], pred_label)
    confusion_mat = get_confusion_matrix(predictions['real'], predictions['samples'], pred_label)
    print(f"{name} F1 SCORE: {f1}, ACCURACY: {accur}")
...
def run_baseline_threshold_experiment(criterion_fn, name, n_samples=500, target_fpr=0.01):
    torch.manual_seed(0)
    np.random.seed(0)
    ...
    accur = get_accuracy(predictions['real'], predictions['samples'], pred_label)
    confusion_mat = get_confusion_matrix(predictions['real'], predictions['samples'], pred_label)
    print(f"{name}_threshold F1 SCORE: {f1}, ACCURACY: {accur}")
...
def eval_supervised(data, model, target_fpr=0.01):
    ...
    _, threshold_val, pred_label = get_threshold(real_preds, fake_preds, target_fpr)
    
    f1 = get_f1_metrics(real_preds, fake_preds, pred_label)
    accur = get_accuracy(real_preds, fake_preds, pred_label)
    confusion_mat = get_confusion_matrix(real_preds, fake_preds, pred_label)
    print(f"{model} F1 SCORE: {f1}, ACCURACY: {accur}")
    #end
...
if __name__ == '__main__':
    ...
    parser.add_argument("--perturb_method", type=str, default="detectgpt")
    parser.add_argument("--noisy_level", type=float, default=None)
    ...
    if "noise" in args.perturb_method:
        assert args.noisy_level is not None, "Must provide a noisy_level as --noisy_level"
    ...
    SAVE_FOLDER = f"tmp_results/{output_subfolder}{args.perturb_method}-{base_model_name}{scoring_model_string}-{args.mask_filling_model_name}-{sampling_string}/{START_DATE}-{START_TIME}-{precision_string}-{args.pct_words_masked}-{args.n_perturbation_rounds}-{args.dataset}-{args.n_samples}-{args.noisy_level}"
